...
def runDT():
	...
	comdir = '/home/xxm/Desktop/EMSE/crawlerCommit'
	...
	for root,dirs,files in os.walk(comdir):
		for file in files:
			if file.endswith(".json"):
				...
				dynfilepath = comfilepath.replace("crawlerCommit","DTResult")
				comfile = json.load(open(comfilepath,"r"))
				dynfile = json.load(open(dynfilepath,"r"))

				for pullid in comfile:
					for sha in comfile[pullid]:
						filelist = comfile[pullid][sha][0]
						statelist = comfile[pullid][sha][1]
				...
				for pullid in comfile:
					for sha in comfile[pullid]:
						...
						if decidepy(filelist) and statelist:
							...
							# print(pullid,decidepy(filelist),filelist,dynState,statelist,commitState)
				# print(ORdic)
				oddsratio, pvalue = stats.fisher_exact([[ORdic["DTF"], ORdic["DTS"]], [ORdic["NDTF"], ORdic["NDTS"] ]])
				print(file,"fisher_exact",oddsratio, pvalue)
				RQ11list.append((file.replace(".json",""),oddsratio, pvalue))
	return RQ11list
